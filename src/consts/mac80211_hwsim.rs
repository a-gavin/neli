...
pub enum Mac80211HwsimCmd {
    ...
    /// `Mac80211HwsimAttr::AddrTransmitter`, `Mac80211HwsimAttr::AddrReceiver`,
    /// `Mac80211HwsimAttr::Frame`, `Mac80211HwsimAttr::Flags`, `Mac80211HwsimAttr::RxRate`,
    /// `Mac80211HwsimAttr::Signal`, `Mac80211HwsimAttr::Cookie`, `Mac80211HwsimAttr::Freq` (optional)
...
pub enum Mac80211HwsimAttr {
    ...
    Receiver = libc::HWSIM_ATTR_ADDR_RECEIVER as u16,
    /// MAC address of the radio device that the frame was broadcasted from
    Transmitter = libc::HWSIM_ATTR_ADDR_TRANSMITTER as u16,
    /// Data array
    ...
    /// `u32` attribute used with the `Mac80211HwsimCmd::CreateRadio` command giving the
    /// number of channels supported by the new radio
    Channels = libc::HWSIM_ATTR_CHANNELS as u16,
    /// `u32` attribute used with `Mac80211HwsimCmd::DestroyRadio` only to destroy a radio
    ...
    /// Used with the `Mac80211HwsimCmd::CreateRadio` command to force use of channel contexts even
    /// when only a single channel is supported
    UseChanctx = libc::HWSIM_ATTR_USE_CHANCTX as u16,
    /// Used with the `Mac80211HwsimCmd::CreateRadio` command to force radio removal when process that
    ...
    /// Nested attribute used with `Mac80211HwsimCmd::CreateRadio` to provide peer measurement
